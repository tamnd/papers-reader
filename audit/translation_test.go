...
// L20 is the rule the GAN paper asked for. Footnote 4 of its introduction
// says the code is available at an address, written as prose, and all three
// translations came back with that address as a Markdown link whose text and
// target are both the address.
func TestL20ReportsALinkTheEnglishDoesNotHave(t *testing.T) {
	const prose = "\n[^4]: The code is at http://example.org/code.\n"
	const link = "\n[^4]: The code is at [the code](http://example.org/code).\n"
	for _, c := range []struct {
		name  string
		en    string
		tr    string
		...
		{
			"the address stands as prose",
			englishBody + prose,
			viBody + "\n[^4]: Mã nguồn ở http://example.org/code.\n",
			false,
		},
		{
			"the address came back as a link",
			englishBody + prose,
			viBody + "\n[^4]: Mã nguồn ở [http://example.org/code](http://example.org/code).\n",
			true,
		},
		{
			"the English wrote the link itself",
			englishBody + link,
			viBody + link,
			false,
		},
		...
		t.Run(c.name, func(t *testing.T) {
			res := result(t, pairOf(t, corpus.VI, c.en, c.tr), "L20")
			if res.Failed() != c.fails {
				t.Errorf("L20 failed=%v, want %v: %v", res.Failed(), c.fails, res.Findings)
			}
		})
	}
}
